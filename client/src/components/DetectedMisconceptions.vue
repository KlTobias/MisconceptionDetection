<template>
  <div class="flex flex-col">
    ...
    <div class="flex items-center mt-3" v-else-if="responseStore.state === ANALYZINGSTATE.ANALYZING">
      <div role="status">
        <svg aria-hidden="true" class="inline w-5 h-5 text-neutral-tertiary animate-spin fill-[#007bff]"
      ...
      <div class="ml-3 text-[rgb(var(--secondary))] italic">
    ...
    <div class="mt-3" v-else-if="responseStore.state === ANALYZINGSTATE.FINISHED">
      <div v-if="responseStore.response && responseStore.response.length">
        <Misconception v-for="(m, idx) in responseStore.response" :key="m.name + '-' + idx" :misconception="m"
          @highlight-lines="onHighlightLines" />
      </div>
      <div v-else class="text-gray-600">No misconceptions found.</div>
    </div>

    <div class="mt-3" v-if="responseStore.state === ANALYZINGSTATE.FINISHED">
      <button @click="clear" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Clear all</button>
    </div>
  </div>
</template>
